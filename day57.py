...
# ChromaDB
chroma = chromadb.PersistentClient(path="./chroma_db_57")
...
def load_kb():
    collection = chroma.get_or_create_collection("transportation_57")
...
def load_session(session_id):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT messages, intent FROM sessions_v2 WHERE session_id = %s", (session_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
    return {"messages": row[0], "intent": row[1]} if row else {"messages": [], "intent": ""}

def save_session(session_id, messages, intent):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO sessions_v2 (session_id, messages, intent, msg_count, updated_at)
        VALUES (%s, %s, %s, %s, NOW())
        ON CONFLICT (session_id) DO UPDATE SET
            ...
            updated_at = NOW()
    """, (session_id, json.dumps(messages), intent, len(messages)))
    conn.commit()
    cur.close()
    conn.close()
...
print(f"\n🚗 Production System v1 | Session: {session_id}")
print("LangGraph + RAG + PostgreSQL\n")
